...
import { Search, Filter, Plus, Pencil, Trash2, ChevronLeft, ChevronRight, User, FileText, Loader2 } from "lucide-react";
import Link from "next/link";
import { initials, getBadgeColor, getAvatarColor } from "@/lib/dummy-data";
import MapelModal from "@/components/guru-mapel(admin)/MapelModal";
import ConfirmDeleteModal from "@/components/ui/ConfirmDeleteModal";
import Toast from "@/components/ui/Toast";

type Guru = { id: string; nama: string; nip: string; email: string; mapel: string };
type MataPelajaran = { id: string; nama: string; guruPengampu: string };
...
export default function GuruMapelPage() {
  ...
  async function fetchAll() {
    setLoading(true);
    const [guruRes, mapelRes] = await Promise.all([fetch("/api/guru"), fetch("/api/mapel")]);
  ...
  async function handleMapelSubmit(data: Omit<MataPelajaran, "id">) {
    if (editMapel) {
      const res = await fetch(`/api/mapel/${editMapel.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(data),
      });
      const updated = await res.json();
      setMapel((prev) => prev.map((m) => (m.id === editMapel.id ? updated : m)));
  ...
  async function handleConfirmDelete() {
    ...
    if (deleteTarget.kind === "guru") {
      await fetch(`/api/guru/${deleteTarget.data.id}`, { method: "DELETE" });
      setGuru((prev) => prev.filter((g) => g.id !== deleteTarget.data.id));
      showToast("Guru berhasil dihapus");
    } else {
      await fetch(`/api/mapel/${deleteTarget.data.id}`, { method: "DELETE" });
  ...
  return (
    <div>
      ...
      {loading ? (
        <div className="mt-5 flex items-center justify-center gap-2 rounded-2xl border border-border bg-surface py-16 text-muted">
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  ...
                  {filteredGuru.length === 0 && (
                    <tr><td colSpan={6} className="px-6 py-10 text-center text-muted">Belum ada data guru. Klik "Tambah Guru" untuk menambahkan.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  ...
                  {filteredMapel.length === 0 && (
                    <tr><td colSpan={4} className="px-6 py-10 text-center text-muted">Belum ada mata pelajaran. Klik "Tambah Mapel" untuk menambahkan.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
